...
DS::Result<void> Main(int argc, char** argv)
{
    ...
    ghc::filesystem::file_time_type lastWriteTime = ghc::filesystem::last_write_time(yamlPaths[0], ec);
    ...
    std::string cmd =   runcpp2Path + " run --log-level error -c -l " + 
                        EscapePath("./DefaultYAMLs/DefaultUserConfig.yaml ") + embed2CPath.string() +
