...
namespace
{
    ...
    bool ParseOutputTypes(  const std::string& subNodeKey,
                            ...
                            std::unordered_map<PlatformName, OutputTypeInfo>& outInfos);
...
namespace Data
{
    ...
    struct StageInfo
    {
        ...
        inline bool ParseYAML_Node(YAML::ConstNodePtr node, std::string outputTypeKeyName)
        {
            ssLOG_FUNC_DEBUG();
            ...
            if(ExistAndHasChild(node, "PreRun"))
            {
                ...
                {
                    std::string key = preRunNode->GetMapKeyScalarAt<std::string>(i)
        ...
        inline std::string ToString(std::string indentation, std::string outputTypeKeyName) const
        {
            ssLOG_FUNC_DEBUG();
        ...
        inline bool Equals(const StageInfo& other) const
        {
...
namespace
{
                            ...
                            std::unordered_map<PlatformName, OutputTypeInfo>& outInfos)
    {
        ...
        for(int i = 0; i < outputTypesSubNode->GetChildrenCount(); ++i)
        {
            ...
            for(int j = 0; j < runPartsNode->GetChildrenCount(); ++j)
            {
                ...
                if(currentType == "Once")
                    outInfos[platformName].RunParts.back().Type = RunPart::RunType::ONCE;
                else if(currentType == "Repeats")
                    outInfos[platformName].RunParts.back().Type = RunPart::RunType::REPEATS;
                else
                {
                                    ...
                                    std::string& outString)
    {
        ...
        for(auto it = toStringMap.begin(); it != toStringMap.end(); ++it)
        {
            ...
            {
                static_assert(static_cast<int>(RunPart::RunType::COUNT) == 2, "Add new RunType");
                
                RunPart::RunType currentType = it->second.RunParts.at(i).Type;
                outString +=    indentation + 
                                "        -   Type: " + 
                                (currentType == RunPart::RunType::ONCE ? 
                                "Once" : 
                                "Repeats") + 
                                "\n";
